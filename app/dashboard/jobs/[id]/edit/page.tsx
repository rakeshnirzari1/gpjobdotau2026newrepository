...
export default async function EditJob({ params }: { params: { id: string } }) {
  ...
  // Check if job is already paid - if so, no payment required for edits
  const paymentRequired = !job.is_paid
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="bg-white p-8 rounded-lg shadow-md">
        <JobForm practice={practice} job={job} paymentRequired={paymentRequired} />
      </div>
    </div>
  )
}
