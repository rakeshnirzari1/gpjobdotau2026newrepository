...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!jobId || !sessionId) {
      return NextResponse.json({ error: "Missing jobId or sessionId" }, { status: 400 })
    }
    ...
    try {
      ...
      if (session.payment_status === "paid") {
        ...
        return NextResponse.json({ success: true, isPaid: false, paymentStatus: session.payment_status })
      }
    } catch (error: any) {
